...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: python plot_data.py <file_path> <type> <human>')
        print(
            'Example: python plot_data.py gpt_api_task/results/left_right'
            '/gpt4o_topP05.txt left_right True')
        sys.exit(1)
    else:
    ...
    # If `human` is True, create a second y-axis and plot the human rt
    if human:
        ax2 = ax1.twinx()
        if type == 'left_right':
        ...
        ax2.errorbar(angles, human_rt, yerr=human_error, fmt='g-s',
                     linewidth=2.5,
