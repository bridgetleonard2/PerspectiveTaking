...
stim_dir = '..\..\images\bitmap\left_right';
...
responses = NaN(3, nTrials); % Initialize a 2xN array filled with NaNs
...
for trial = 1:nTrials
    ...
    % Deterine whether there is a arrow cue and the contrast of the trial
    ...
    for frame = 1:nFramesStim
        ...
        if pressed
            ...
            if leftPressed || rightPressed
                [responses(1, trial), responses(3, trial)] = handle_response(firstPress, leftKey, rightKey, cube_sphere, escapeKey, window);
...
scriptContent = fileread('cube_LeftRight.m');
...
%%
function [response, correct] = handle_response(firstPress, leftKey, rightKey, cube_sphere, escapeKey, window);
    if firstPress(leftKey)
        response = 1;  % or 'left' if using cell array
        if cube_sphere == 1
            correct = 1;
        elseif cube_sphere == 2
            correct = 0;
        end
...
function plotResults(responses, trialOrder)
    ...
    for i = 1:length(unique_orientations)
        ...
        indices = (orientations == unique_orientations(i) & correctTrials == 1);
        
        % Calculate the mean reaction time for the current orientation
        mean_reactionTimes(i) = nanmean(reactionTimes(indices));
    end
